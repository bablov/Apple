...
namespace Baebae.Apple.NPOI
{
    public static class NPOIExcel
    {
        ...
        public static IRow CreateRow(this ISheet sheet, int rowNum, float rowHeight)
        {
        ...
        /// <returns></returns>
        public static ICell CreateCells(this IRow row, ICellStyle cellStyle, int cellNum, string cellValue)
        {
        ...
        ///  添加单元格为下拉
        /// </summary>
        /// <param name="sheet"> 要加入列表的sheet</param>
        /// <param name="itemSheet">选项 sheet. </param>
        /// <param name="headerCell"> 标题单元格</param>
        /// <param name="items">列表项</param>
        public static void AddValidation(this ISheet sheet, ISheet itemSheet, ICell headerCell, List<string> items)
        {
            // 新建行
            ...
            {
                IRow row = itemSheet.CreateRow(i);
            ...
            // 格式 Sheet2!$A$1:$A$65535
            var dvConstraint = dvHelper.CreateFormulaListConstraint(
                $"{itemSheet.SheetName}!$A${ 1}:$A${items.Count}");
        ...
        public static ICellStyle CreateStyle(
           this IWorkbook workbook,
            ...
            bool isBold = false,
            string fontName = "宋体",
            bool isAddBorderColor = true,
            bool isItalic = false,
            bool isLineFeed = false,
            bool isAddCellBackground = false,
            FillPattern fillPattern = FillPattern.NoFill,
            short cellBackgroundColor = HSSFColor.Yellow.Index,
            short fontColor = HSSFColor.Black.Index,
            FontUnderlineType underlineStyle = FontUnderlineType.None,
            FontSuperScript typeOffset = FontSuperScript.None,
            bool isStrikeout = false)
        {
            ICellStyle cellStyle = workbook.CreateCellStyle(); //创建列头单元格实例样式
